...
export class AuthGuard implements CanActivate {
  constructor(@Inject(Authentication) private _authentication: Authentication,
  ...
  public canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
    ...
    if (route.data && route.data.roles && route.data.roles.length > 0) {
      ...
      for (let i = 0, roles = route.data.roles; i < roles.length; i++) {
        if ((roles[i] === 'guest') ||
           (this._authentication.user &&
            this._authentication.user.roles !== undefined &&
            this._authentication.user.roles.indexOf(roles[i]) !== -1)) {
          allowed = true;
          break;
        }
      }
      ...
      if (!allowed) {
        if (this._authentication.user !== undefined &&
            typeof this._authentication.user === 'object') {
